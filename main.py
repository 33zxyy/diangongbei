...
def load_attachment1(path: Path):
    ...
    raw_sheets = {s: normalize_columns(pd.read_excel(path, sheet_name=s)) for s in xls.sheet_names}
    ...
    col_income = find_col(merged.columns, ['人均月收入', '收入'])
    col_p12 = find_col(merged.columns, ['p12', '自理转半失能', '自理→半失能'])
    col_p23 = find_col(merged.columns, ['p23', '半失能转失能', '半失能→失能'])
...
def load_attachment2(path: Path):
    ...
    col_r = find_col(merged.columns, ['需求次数', '需求频次', '次数'])
...
def forecast_population(base_df: pd.DataFrame):
    ...
    for _, row in base_df.iterrows():
        ...
        for t in range(YEARS + 1):
            T = n1 + n2 + n3
            records.append({'小区': c, '年份': t, '自理': n1, '半失能': n2, '失能': n3, '总人数': T, 'p12': p12, 'p23': p23})
            if t < YEARS:
...
def compute_demand(base_df, pred_df, demand_df):
    yr5 = pred_df[pred_df['年份'] == YEARS][['小区', '自理', '半失能', '失能', '人均月收入']].copy() if '人均月收入' in pred_df.columns else None
    if yr5 is None:
        base_income = base_df[['小区', '人均月收入']]
        yr5 = pred_df[pred_df['年份'] == YEARS][['小区', '自理', '半失能', '失能']].merge(base_income, on='小区', how='left')

    long_pop = yr5.melt(id_vars=['小区', '人均月收入'], value_vars=['自理', '半失能', '失能'],
                        var_name='老人类型', value_name='第5年人数')
    ...
    theta_df['B_i_m'] = theta_df['lambda_m'] * theta_df['人均月收入']
    theta_df['theta'] = np.where(theta_df['E_m'] > 0, np.minimum(1.0, theta_df['B_i_m'] / theta_df['E_m']), 1.0)
...
def main():
    ...
    pred_yr5 = pred_df[pred_df['年份'] == YEARS][['小区', '自理', '半失能', '失能']]
    pred_yr5 = pred_yr5.merge(base_df[['小区', '人均月收入']], on='小区', how='left')
    demand_detail, theta_df = compute_demand(base_df, pred_yr5.assign(年份=YEARS), demand_df)
